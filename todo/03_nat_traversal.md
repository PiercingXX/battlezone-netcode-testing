...
## Goal
Improve UDP hole-punch success rate so peers that currently fall back to the relay server (`RELAY_HOST:1339`) instead achieve direct P2P connections.  Hook the `connect` / send path in the proxy to implement more aggressive simultaneous-open and shorten the time spent in the relay.
...
In session 1 (232149Z) player **Player_C** (client ID `CLIENT_C`) failed direct P2P and was looped through `RELAY_HOST:1339`, adding ≈154 ms latency.  The relay fallback happened *twice* in that session (19:23 and 19:26), suggesting the direct route was being renegotiated and failing again.  Player_C also had a 999,592 µs (≈1 s) clock offset, which is typical of high-latency relay paths where round-trip timing inflates the skew estimate.  Player_C dropped out of the session at 19:25:49.

## Observed Relay Failure Sequence (session 1 — 232149Z)

```
2026-03-14 19:23:31.921753 BZRNet P2P FAILED WAN Connect For Client CLIENT_C, ATTEMPTING RELAY
2026-03-14 19:23:32.032755 BZRNet P2P Starting RELAY Connect For Client CLIENT_C - P2P Relay Response
2026-03-14 19:23:33.198776 BZRNet P2P Completed RELAY Connect For Client CLIENT_C, using address RELAY_HOST:1339
...
2026-03-14 19:26:36.141110 BZRNet P2P FAILED WAN Connect For Client CLIENT_C, ATTEMPTING RELAY
2026-03-14 19:26:36.252112 BZRNet P2P Starting RELAY Connect For Client CLIENT_C - P2P Relay Response
2026-03-14 19:26:37.411133 BZRNet P2P Completed RELAY Connect For Client CLIENT_C, using address RELAY_HOST:1339
```
...
- The relay server `RELAY_HOST:1339` — is it the official BZRNet relay or a community one?  Check whether it supports TURN-style channel data so future work could improve relay throughput.
...
|------------|------------------------|---------------|--------------------------|
| Player_C   | CLIENT_C               | 999,592 µs    | Relay, dropped at 19:25  |

## Done When
A test session with Player_C (or a volunteer behind symmetric NAT) shows `Completed … direct` in BZLogger instead of `Completed RELAY Connect`, or relay handoff latency drops below 50 ms in the worst case.
